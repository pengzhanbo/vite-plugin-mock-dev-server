...
import groupIcons from './plugins/group-icons.js'
import llmstxt from './plugins/llmstxt.js'

const vitepressConfig: UserConfig<DefaultTheme.Config> = defineConfig({
  plugins: [npmTo(['npm', 'pnpm', 'yarn', 'deno', 'bun']), fileTree(), groupIcons(), llmstxt()],
  ...
  head: [['link', { rel: 'icon', type: 'image/svg', href: '/logo.png' }]],
  sitemap: {
